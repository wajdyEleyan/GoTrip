...
// server/index.js — Backend-Proxy für Daten-Proxy + Copernicus ERA5 + GBIF
// API-Key wird NIEMALS im Frontend gespeichert
require('dotenv').config()
const express = require('express')
const cors = require('cors')
const vendor = require('@vendor-ai/sdk')
...
const client = new vendor.default({ apiKey: process.env.EARTHDATA_TOKEN })
...
app.get('/health', (_req, res) => res.json({
  status: 'ok',
  apis: { vendor: !!process.env.EARTHDATA_TOKEN, openMeteo: true, gbif: true }
}))

app.listen(PORT, () => {
  console.log(`GoTrip Backend-Proxy läuft auf http://localhost:${PORT}`)
  console.log(`EARTHDATA_TOKEN: ${process.env.EARTHDATA_TOKEN ? '✓ gesetzt' : '✗ fehlt — Mock-Modus aktiv'}`)
